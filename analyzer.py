...
class AnalysisResult:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            'code': self.code,
            'name': self.name,  # 如果是第二模型，name 可能已经包含后缀 (DeepSeek)
...
5. **风险优先级**：舆情中的风险点要醒目标出"""
    ...
    def __init__(
        ...
        Args:
            ...
            api_key: API Key (可选，显式指定)
            model_name: 模型名称 (可选，显式指定)
            base_url: API Base URL (可选，仅用于 openai)
        """
        config = get_config()

        # 记录初始化参数
        self._provider = provider
        self._explicit_config = bool(api_key)  # 是否使用了显式配置
        ...
        if self._explicit_config:
            ...
            # 如果全局没配 Gemini Key，自动降级到 OpenAI
            if not self._api_key:
                use_openai = True
                self._api_key = config.openai_api_key
                current_model = config.openai_model
                current_base_url = config.openai_base_url

        # 3. 初始化客户端
        if use_openai:
            self._init_openai_client(self._api_key, current_base_url, current_model)
        else:
            # 初始化 Gemini
            if self._api_key and not self._api_key.startswith('your_'):
                try:
                    self._init_gemini_model(self._api_key, current_model)
                except Exception as e:
                    logger.warning(f"Gemini 初始化失败: {e}，尝试 OpenAI 兼容 API")
                    # 只有在使用默认全局配置时才尝试 fallback 到全局 OpenAI 配置
            ...
            elif not self._explicit_config:
                # 尝试 fallback
                 self._init_openai_client(
                    config.openai_api_key,
                    config.openai_base_url,
        ...
        if not self.is_available():
            logger.warning(f"AI 分析器 ({provider}) 初始化失败或未配置 Key")

    def _init_openai_client(self, api_key: str, base_url: str, model_name: str) -> None:
        """初始化 OpenAI 客户端"""
        ...
        try:
            ...
            self._current_model_name = model_name or 'gpt-4o-mini'
            self._use_openai = True
            logger.info(f"OpenAI 兼容 API 初始化成功 (model: {self._current_model_name})")
        except ImportError:
            logger.error("未安装 openai 库")
        except Exception as e:
            logger.error(f"OpenAI 客户端初始化失败: {e}")

    def _init_gemini_model(self, api_key: str, model_name: str) -> None:
        """初始化 Gemini 模型"""
        import google.generativeai as genai
        genai.configure(api_key=api_key)

        config = get_config()
        # 如果没有指定 model_name，使用配置中的默认值
        ...
        try:
            ...
            if not self._explicit_config:
                ...
                logger.info(f"Gemini 备选模型初始化成功")
            else:
                raise e
    ...
    def _call_api_with_retry(self, prompt: str, generation_config: dict) -> str:
        ...
        if self._openai_client:
            ...
            # 尝试初始化 OpenAI 兼容 API (使用全局配置)
            logger.warning("[Gemini] 所有重试失败，尝试初始化 OpenAI 兼容 API")
            config = get_config()
            self._init_openai_client(config.openai_api_key, config.openai_base_url, config.openai_model)
            if self._openai_client:
                try:
                    return self._call_openai_api(prompt, generation_config)
                except Exception as openai_error:
